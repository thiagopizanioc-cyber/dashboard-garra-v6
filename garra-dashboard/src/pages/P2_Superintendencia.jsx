...
import { KpiCard, Card, FunilBar, AlertaBanner, Semaforo } from '../components/Shared';
import { fmt, consolidar, semaforo, pontosDeAtencao } from '../utils/index';

export function P2_Superintendencia({ data, target, setTarget, setPage }) {
  ...
  return (
    <div className="page">
      ...
      {!cons ? <p className="empty">Sem dados para esta superintendência.</p> : <>
              ...
              return (
                <div key={g} className="engaj-row">
                  <div className="engaj-header">
                    <span className="engaj-nome">{g}</span>
                    <span className="engaj-pct" style={{color: pct>=80?'#34d399':pct>=50?'#fbbf24':'#f87171'}}>{pct}%</span>
                  </div>
                  <div className="engaj-bar-bg">
                    <div className="engaj-bar-fill" style={{width:`${pct}%`, background: pct>=80?'#34d399':pct>=50?'#fbbf24':'#f87171'}}/>
                  </div>
                  {semDados.length>0 && (
                    <div className="engaj-warn">⚠️ Sem dados: {semDados.map(c=>c.corretor).join(', ')}</div>
                  )}
                </div>
              );
            })}
        ...
        {/* Tabela de gerentes com semáforos */}
            ...
            <table className="data-table">
              ...
              <tbody>
                {Object.entries(gerentesMap).sort((a,b)=>{
                  ...
                  return (
                    <tr key={g}>
                      <td className="td-nome">{g}</td>
                      <td>{c.ativos}/{c.total}</td>
                      {metricas.map(m=>{
                        const val = m.fn({...c});
                        const medVal = m.fn(media);
                        const nv = semaforo(val, medVal, false);
                        return (
                          <td key={m.key}>
